...
#pragma unroll 	
	...
	unsigned char my_thd_colors = 0;//the colos that this thread will do
	...
	const uint32_t end_row = min(blockIdx.x * blockDim.x + blockDim.x, NumRow);
...
/*#pragma unroll 
	for(uint32_t i= threadIdx.x; i< blockDim.x; i+=blockDim.x){
		sh_set[i] = false;
	}*/
	sh_set[threadIdx.x] = false;//the size = blockDim.x 
	...
	if(threadIdx.x == 0){
		...
		block_start_col_id = my_offset_start;//enable if using shared mmeory
	...
	/*if(tid < NumRow){
		printf("\n tid= %d, threadIdx.x= %d, blockIdx.x= %d, block_myNNZ=%d, block_start_col_id=%d, my_offset_start= %d, my_offset_end= %d, start_row= %d, end_row= %d, block_start_col_id=%d, \n", tid, threadIdx.x, blockIdx.x, block_myNNZ, block_start_col_id, my_offset_start, my_offset_end, start_row, end_row,block_start_col_id);
	}*/


	//my_offset_start -= block_start_col_id; //enable for shred memory 
